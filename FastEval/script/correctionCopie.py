...
image = cv2.imread('imgDiviser.png')
...
if docCnt.all()!=None :
	...
	for c in cnts:
		...
		if w >= 20 and h >= 20 and ar >= 0.9 and ar <= 1.1:
			...
			for i in range(len(tabXY)):
				if (tabXY[i][0]-20)<=xx and xx<=(tabXY[i][0]+20) and (tabXY[i][1]-20)<=yy and yy<=(tabXY[i][1]+20) :
					trouve=1
			if(trouve==0 or len(tabXY)==0):
				tabXY.append([xx, yy])
	...
	# ANSWER_KEY = {0: [0,2], 1: [2], 2: [2], 3: [0], 4: [1], 5: [1,2], 6: [1], 7: [2], 8: [1,2], 9: [2,1], 10: [2], 11: [1], 12: [0]}
	ANSWER_KEY = rep
	...
	#image1 = Image.open(r'C:\Users\melan\PycharmProjects\untitled\result.jpg')
	#im1 = image1.convert('RGB')
	# convertion du résultat en pdf
	#im1.save(r'C:\Users\melan\PycharmProjects\untitled\ExamenCorrige.pdf')
	cv2.waitKey(0)
else :
	print(-1)
